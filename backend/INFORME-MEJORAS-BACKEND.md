...
- [ ] **Revisar payloads de endpoints + UX de redes lentas (2026-06-27).** Auditar qué **cantidad de información** devuelve cada endpoint (¿campos de más en los DTOs?, ¿se mandan relaciones/objetos completos cuando alcanzaría con ids/resúmenes?, ¿listas sin paginar?) y adelgazar las respuestas. Relevante ahora que `/api` es **network-only** (sin fallback de cache): en redes lentas, payloads grandes pegan más fuerte. Evaluar soluciones: DTOs más livianos / selección de campos, paginación donde falte, compresión (gzip/brotli en el backend), estados de carga/skeletons y reintentos en el front, y revisar el `networkTimeout` percibido. Empezar por los endpoints del hot path (dashboards cliente/proveedor, `/api/trabajos/*`, perfil).
