...
- [~] **Payloads de endpoints + UX de redes lentas (2026-06-27).** Auditoría del hot path + primer batch HECHO:
  ...
  - **[ ] Pendiente — DTO "resumen" para listas (#3):** `TrabajoResponseDTO` se usa igual en detalle y en listas → las listas (`/pendientes`, `/cliente`, `/en-cola`, `/completados`) mandan `fotos`, coords (cliente+destino) y `descripcion` completa por item. Crear un DTO liviano de lista. (Refactor de controllers/services del hot path.)
  - **[ ] Pendiente — UX redes lentas:** skeletons/loaders, reintentos con backoff, timeouts percibidos. Relevante porque `/api` quedó network-only (sin fallback de cache).
