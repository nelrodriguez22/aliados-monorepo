...
- [x] #7 EAGER→LAZY (2026-06-17, sesión 2) — **implementado, PENDIENTE VERIFICAR en prod con OSIV off**:
  ...
  - **Fix post-deploy #1**: `LazyInitializationException` en `Oficio` al serializar. Causa: el DTO embebe la entidad `Oficio` (`TrabajoResponseDTO`/`UserResponseDTO`) y los mappers la guardaban como proxy sin tocar ningún campo → nunca se inicializaba → fallaba al serializar fuera de la tx. Solución: `Hibernate.initialize(getOficio())` antes de embeberla, en `TrabajoService.mapToDTO`/`mapToDTOOptimized` y `UserService.mapToDTO`. (Único embed de entidad en DTOs = `Oficio`; el resto son escalares.)
  - **Falta**: re-deploy + recorrer endpoints de listado/detalle mirando logs (no debe haber `LazyInitializationException`; contar queries para confirmar que no hay N+1 residual).
