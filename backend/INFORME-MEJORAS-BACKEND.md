...
- [x] #19 Logging bajado (2026-06-18). `logging.level.org.hibernate.SQL` DEBUG → **WARN** (ya no loguea cada query en prod: ruido/overhead/fuga). `ddl-auto=validate` y `show-sql=false` ya estaban (Fase 1). ⚠️ **Riesgo adicional detectado:** `mudanza.ratio-tiempo` default es **180.0 (testing: 1 min real = 3hs)**, no 1.0 — si prod no setea `MUDANZA_RATIO_TIEMPO`, corre en modo testing. Verificar que la env var esté seteada en Railway (no se tocó el default para no romper el setup actual).
- [ ] #17 Localidad case-sensitive (`=` vs `equalsIgnoreCase`) → normalizar al guardar.
- [ ] #18 `getMudanzasPendientesProveedor` provisional (devuelve todas a cualquier proveedor).
