...
- [x] **UX: pantalla de propuesta tras aceptar — RESUELTO (2026-06-26).** Al aceptar una propuesta, `ClientProposal.tsx` quedaba mostrando "Esta propuesta ya no está disponible / El estado del trabajo cambió" en vez de ir al seguimiento. **Causa:** el `onSuccess` navegaba con `ROUTES.CLIENT.TRACKING(data.id)`, pero el endpoint `aceptar-propuesta` no devuelve el trabajo con `id` → `data.id` undefined → navegación fallida → re-render con `estado !== 'PROPUESTO'` → cartel de fallback. **Fix:** navegar con `jobId` (param de la URL, siempre definido). `tsc` OK. _Pendiente: redeploy FE._
